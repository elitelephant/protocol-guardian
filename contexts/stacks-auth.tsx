...
import { AppConfig, UserSession, authenticate, UserData, connect } from '@stacks/connect'
import { STACKS_MAINNET, STACKS_TESTNET, StacksNetworks } from '@stacks/network'
...
interface StacksAuthProviderProps {
  children: ReactNode
  testnet?: boolean // Default to mainnet, set true for testnet
}

export function StacksAuthProvider({ children, testnet = false }: StacksAuthProviderProps) {
  const [userSession, setUserSession] = useState<UserSession | null>(null)
  const [userData, setUserData] = useState<UserData | null>(null)
  const [isLoading, setIsLoading] = useState(true)
  
  // Configure network (mainnet by default, testnet for development)
